...
session_start();
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($username) && !empty($password) && !is_numeric($username))
		{
			...
			//read from database
			$query = "select * from userss where username = '$username' limit 1";
			...
			if($result)
			{
				if($result && mysqli_num_rows($result) > 0)
				{
					...
					if($user_data['password'] === $password)
					{
						...
						$_SESSION['id_ens'] = $user_data['id_ens'];
						...
						$sql = "INSERT INTO login_history (username, login_time) VALUES ('$username', NOW())";
						$conn->query($sql);
...
.wrapper{
    background: #ececec,#019477;
    background: -webkit-linear-gradient(-135deg,hsl(0, 0%, 6%), hsl(168, 40%, 32%),hsl(168, 43%, 64%));
        ...
        <div class="row">
            ...
            <div class="col-md-6 right">
                ...
                <div class="input-box">
                   ...
                   <div class="signin">
                    <span>Vous avez déjà un compte? <a href="registration.php">S'inscrire</a></span>
                   </div>
                </div>  
            </div>
        </div>
